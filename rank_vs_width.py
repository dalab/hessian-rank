...
n_train = 100
bs = args.batch_size
widths = [i * 4 for i in range(1, 15)]
...
store_path = dir_path + '/results/store/width/'
store_path += args.loss + '/' + args.dataset + '/'

# Fix path to store intermediate results
temp_path = dir_path + '/results/temporary/width/'
temp_path += args.loss + '/' + args.dataset + '/'
...
for width in widths:
    ...
    print('Iteration ' + str(counter) + ' out of ' + str(len(widths)))
    counter += 1
    print(counter)


# Store the results in the folder
rank_F_frame = pd.DataFrame({'width': widths, 'Rank': jnp.array(ranks['rank_F'])}, dtype=float)
rank_F_frame.to_pickle(path=store_path + 'rank_F')
rank_L_frame = pd.DataFrame({'width': widths, 'Rank': jnp.array(ranks['rank_L'])}, dtype=float)
rank_L_frame.to_pickle(path=store_path + 'rank_L')
rank_outer_frame = pd.DataFrame({'width': widths, 'Rank': jnp.array(ranks['rank_outer'])}, dtype=float)
rank_outer_frame.to_pickle(path=store_path + 'rank_outer')

num_params_frame = pd.DataFrame({'width': widths, 'Num': jnp.array(num_params)}, dtype=float)
num_params_frame.to_pickle(path=store_path + 'num_params')

preds_F_frame = pd.DataFrame({'width': widths, 'Pred': jnp.array(preds['rank_F'])}, dtype=float)
preds_F_frame.to_pickle(path=store_path + 'pred_F')
preds_L_frame = pd.DataFrame({'width': widths, 'Pred': jnp.array(preds['rank_L'])}, dtype=float)
preds_L_frame.to_pickle(path=store_path + 'pred_L')
preds_outer_frame = pd.DataFrame({'width': widths, 'Pred': jnp.array(preds['rank_outer'])}, dtype=float)
